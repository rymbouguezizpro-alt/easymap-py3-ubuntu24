...
def boxplot_stats(lista):
	...
	mid = (lenght - 1) / 2.0
	if mid.is_integer():
		per_50 = lista[int(mid)]
	else:
		lower = int(math.floor(mid))
		upper = int(math.ceil(mid))
		per_50 = (lista[lower] + lista[upper]) / 2 

	# Cuartil 25
	p25 = (lenght + 1) / 4.0
	if p25.is_integer():
		per_25 = lista[int(p25)]
	else:
		lower = int(math.floor(p25))
		upper = int(math.ceil(p25))
		per_25 = (lista[lower] + lista[upper]) / 2

	# Cuartil 75
	p75 = (lenght + 1) * 3 / 4.0
	if p75.is_integer():
		per_75 = lista[int(p75)]
	else:
		lower = int(math.floor(p75))
		upper = int(math.ceil(p75))
		per_75 = (lista[lower] + lista[upper]) / 2
...
def Draw_box_plot(table,out):
	...
	size_x_axis = max(min_x_axis, n * box_spacing + 10)
	x_window = size_x_axis + a + b
	y_window = 460
	...
	for idx,position in enumerate(table):
		...
		if idx % label_step == 0 :
			...
			bbox = draw.textbbox((0,0), str(name), font=fnt1)
			w = bbox[2]-bbox[0]
			draw.text((i - w // 2, position_y_axis + 10), str(name), font=fnt1, fill=(0, 0, 0, 0))				
		...
		#Create the boxplot 
		beg = float(position[2]) * step   # cuartil 25
		end = float(position[-2]) * step  # cuartil 75
		# calcular top y bottom para que y1 >= y0
		y_top = position_y_axis - end
		y_bot = position_y_axis - beg
		draw.rectangle([(i-3, y_top), (i+3, y_bot)], fill=(24, 56, 214), outline=None)
	...
	# Convertir a int la coordenada Y
	y_offset = -int(size_y_axis//2 + 20)
	im.paste(ImageOps.colorize(w, (0,0,0), (0,0,0)), (2, y_offset), w)
...
if 100>=lenght_reads>80: group = 1
if lenght_reads>100: group = 2
