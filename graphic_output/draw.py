...
def dens_ovw():
    ...
    len_gnm = 0
    for f in fastalist: 
        if  (f[1]) > 2000000: len_gnm += int(f[1])
    ...
    if args.mut_type == "EMS":
        ...
        for c in range(1,5):
            ...
            bbox = draw.textbbox((0, 0), str(sub_title), font=fnt2)
            w = bbox[2] - bbox[0]
            h = bbox[3] - bbox[1]
            ...
            if max_dens > 0: 
                ...
                while mark_y >= base[1] and mark_y <= base[1]+sub_height:
                    ...
                    bbox = draw.textbbox((0, 0), str(tag_y), font=fnt2)
                    w = bbox[2] - bbox[0]
                    h = bbox[3] - bbox[1]
            ...
            for ind, chr in enumerate(fastalist):
                ...
                if c==4:
                    try:
                        label = str(chr[0]).split('.')[-2]
                        bbox = draw.textbbox((0, 0), label, font=fnt2)
                        w = bbox[2] - bbox[0]
                        h = bbox[3] - bbox[1]
        ...
        for c in range(1,4):
            ...
            for ind, chr in enumerate(fastalist):
                ...
                for ma in range(prev_pos_start, pos_start-10, sep): 
                    draw.line((ma, base[1]+sub_height)+(ma, base[1]+3+sub_height), fill=(0,0,0))
                    if c == 3:
                        bbox = draw.textbbox((0, 0), str(tag), font=fnt2)
                        w = bbox[2] - bbox[0]
                        draw.text((ma-w//2, base[1]+sub_height+8), str(tag), font=fnt2, fill=(0,0,0))
                        tag +=mb_step
                ...
                # Chromosome titles
                if c==3:
                    bbox = draw.textbbox((0, 0), str(chr[0]), font=fnt2)
                    w = bbox[2] - bbox[0]
...
# testing : python3 graphic-output.py  -bsnp genome.fa
# python3 graphic-output.py   -bsnp genome.fa -1 F2_filtered_dens.va -2 F2_control_comparison_dens.va -3 F2_filtered_EMS_dens.va -4 F2_filtered_EMS_hz_dens.va 
    ...
    if args.mut_type == "EMS":
        ...
        for i in fastalist:
            if int(i[1]) > 2000000: 
                ...
                bbox = draw.textbbox((0, 0), str(chrm_title), font=fnt2)
                w = bbox[2] - bbox[0]
        ...
        if int(i[1]) > 2000000: 
            ...
            if args.my_mut == 'af_candidates':
                ...
                for line in binput:
                    if line.startswith('?'):
                        ...
                        chromosome_candidate = chromosome
                        if chromosome == i[0].lower():
                            cr_start_raw = float(sp[2])
                            cr_end_raw = float(sp[3])
                            if float(sp[2]) > 0 :
                                cr_start = float(sp[2])  
                            else:
                                cr_start = 0
                            if  float(sp[3]) < float(i[1]) :
                                cr_end = float(sp[3]) 
                            else:
                                cr_end = float(i[1])
                ...
                if chromosome_candidate == i[0].lower():
                    ...
                    cr_middle = int(((cr_start_raw + cr_end_raw)/2)/scaling_factor_x + 70)
            ...
            if max_contig_len > 1000000  and max_contig_len <= 40000000 :
                ...
                while mbs in range(68, wide-50):
                    ...
                    mbs = int(mbs + 1000000/scaling_factor_x +1)
                    x_tag = x_tag + 1
            ...
            if max_contig_len > 40000000 and max_contig_len <= 90000000:
                ...
                while mbs in range(68, wide-50):
                    ...
                    mbs = int(mbs + 5000000/scaling_factor_x +1)
                    x_tag = x_tag + 5
            ...
            if max_contig_len > 90000000 :
                ...
                while mbs in range(68, wide-50):
                    ...
                    mbs = int(mbs + 25000000/scaling_factor_x +1)
                    x_tag = x_tag + 25
                ...
                while mbs in range(68, wide-50):
                    ...
                    mbs = int(mbs + 100000/scaling_factor_x +1)
                    x_tag = x_tag + 100000
            ...
            bbox = draw.textbbox((0, 0), str(x_title), font=fnt2)
            w = bbox[2] - bbox[0]
            h = bbox[3] - bbox[1]
            draw.text((((wide-120)//2 - w//2 + 70), (int(87/100.0*height))), str(x_title), font=fnt2, fill=(0, 0, 0, 255))
...
def qtl_plot():
    ...
    for i in fastalist:
        if int(i[1]) > 2000000: 
            ...
            #Crop and save image, specifying the format with the extension
            w, h = im.size
            im.crop((0, 60, w-0, h-40)).save(project + '/3_workflow_output/mapping_' + str(i[0]) + '_AF.png')
        ...
        if int(i[1]) > 2000000: 
            ...
            with open(input1, "r") as lines: 
                for l, line in enumerate(lines):
                    ...
                    if i[0].lower() == sp[0].lower() :
                        ...
                        # dAF
                        dAF = float(sp[11])/2
                        dAF_img = int(47.5/100.0*height) - int(dAF/scaling_factor_y) 
                        draw.ellipse((pos_img-2, dAF_img-2, pos_img+2, dAF_img+2), fill=(0, 0, 0))
            ...
            if max_contig_len > 40000000 and max_contig_len <= 90000000:
                ...
                while mbs in range(68, wide-50):
                    ...
                    mbs = int(mbs + 4000000/scaling_factor_x +1)
                    x_tag = x_tag + 4


            if max_contig_len > 90000000 and max_contig_len <= 500000000:
                mbs = int(0/scaling_factor_x) + 68
                x_tag = 0
                while mbs in range(68, wide-50):
                    draw.line((mbs, int(81/100.0*height) ) + (mbs, int(80/100.0*height)), fill=(0, 0, 0, 0), width=1)	
                    ...
                    mbs = int(mbs + 20000000/scaling_factor_x +1)
                    x_tag = x_tag + 20

            if max_contig_len > 500000000:
                mbs = int(0/scaling_factor_x) + 68
                x_tag = 0
                while mbs in range(68, wide-50):
                    draw.line((mbs, int(81/100.0*height) ) + (mbs, int(80/100.0*height)), fill=(0, 0, 0, 0), width=1)	
                    ...
                    mbs = int(mbs + 50000000/scaling_factor_x +1)
                    x_tag = x_tag + 50
            ...
            d.text( (0, 0), "dAF",  font=fnt2, fill=255)
            w=txt.rotate(90,  expand=1)
            im.paste( ImageOps.colorize(w, (0,0,0), (0,0,0)), (2,116),  w)
            ...
            draw.line(( 68 , dAF_img_0 +1) + ( 63 , dAF_img_0 +1 ), fill=(0, 0, 0, 0), width=1)	
            draw.line(( 68 , dAF_img_1 ) + ( 63 , dAF_img_1 ), fill=(0, 0, 0, 0), width=1)	
            draw.line(( 68 , dAF_img_05 ) + ( 63 , dAF_img_05 ), fill=(0, 0, 0, 0), width=1)	
            draw.line(( 68 , dAF_img_025 ) + ( 63 , dAF_img_025 ), fill=(0, 0, 0, 0), width=1)	
            draw.line(( 68 , dAF_img_075 ) + ( 63 , dAF_img_075 ), fill=(0, 0, 0, 0), width=1)	

            draw.text(((32), dAF_img_0-6), ( '-1' ), font=fnt2, fill=(0,0,0,255))
            draw.text(((32), dAF_img_1-8), ( '1' ), font=fnt2, fill=(0,0,0,255))
            draw.text(((32), dAF_img_05-8), ( '0' ), font=fnt2, fill=(0,0,0,255))
            ...
            #Crop and save image, specifying the format with the extension
            w, h = im.size
            im.crop((0, 60, w-0, h-40)).save(project + '/3_workflow_output/mapping_' + str(i[0]) + '_dAF.png')
...
def candidates_zoom():
    ...
    #Peak line
    peak = int((((int(reg_max_real) + int(reg_min_real))/2) - int(reg_min))/scaling_factor_x + 70)
    ...
    if interval_width == 4000000:
        ...
        while mark in range(68, wide-50):
            ...
            bbox = draw.textbbox((0, 0), str(x_tag).strip(), font=fnt2)
            w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    ...
    bbox = draw.textbbox((0, 0), x_title, font=fnt2)
    w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    draw.text((( (wide-120)//2- w//2 +70), (int(87/100.0*height))), (x_title), font=fnt2, fill=(0,0,0,255))
...
def insertions_overview_and_histograms():
    ...
    for c in superlist:
        ...
        for i in c[5]:
            d = abs(int(i/contigs_scaling_factor+contig_xi_coord) - previous_pos_x)
            if d > 21 or previous_chr != c[0]: 
                draw.polygon([((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]-8)), (((int(i/contigs_scaling_factor+contig_xi_coord))+10), (c[2]-18)), ((int(i/contigs_scaling_factor+contig_xi_coord))-10, c[2]-18)], fill = (200, 0, 0, 200))
                draw.line((((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]-8)), ((int(i/contigs_scaling_factor+contig_xi_coord))+10), (c[2]-18)), fill=256, width=1)
                draw.line((((int(i/contigs_scaling_factor+contig_xi_coord))+10), (c[2]-18)) + ((int(i/contigs_scaling_factor+contig_xi_coord))-10, (c[2]-18)), fill=256, width=1) 
                draw.line(((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]-8)) + ((int(i/contigs_scaling_factor+contig_xi_coord))-10, (c[2]-18)), fill=256, width=1) 
                draw.text(((int(i/contigs_scaling_factor+contig_xi_coord) -4), (c[2]-35)), ( str(number)), font=fnt3, fill=(0,0,0,255))
            else:
                draw.polygon([((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]+8)), (((int(i/contigs_scaling_factor+contig_xi_coord))-10), (c[2]+18)), ((int(i/contigs_scaling_factor+contig_xi_coord))+10, c[2]+18)], fill = (200, 0, 0, 200))
                draw.line((((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]+8)), ((int(i/contigs_scaling_factor+contig_xi_coord))-10), (c[2]+18)), fill=256, width=1)
                draw.line((((int(i/contigs_scaling_factor+contig_xi_coord))-10), (c[2]+18)) + ((int(i/contigs_scaling_factor+contig_xi_coord))+10, (c[2]+18)), fill=256, width=1) 
                draw.line(((int(i/contigs_scaling_factor+contig_xi_coord)), (c[2]+8)) + ((int(i/contigs_scaling_factor+contig_xi_coord))+10, (c[2]+18)), fill=256, width=1) 
                draw.text(((int(i/contigs_scaling_factor+contig_xi_coord) -4), (c[2]+20)), ( str(number)), font=fnt3, fill=(0,0,0,255))

            number = number + 1
            previous_pos_x = int(i/contigs_scaling_factor+contig_xi_coord)
            previous_chr = c[0]

        #Chromosome caps
        image_file_left = BytesIO(open("./fonts/left_cap.png",'rb').read())
        image_file_right = BytesIO(open("./fonts/right_cap.png",'rb').read())
    ...
    if mb_max < 40:
        while int(m) in range(100, 951):
            ...
            bbox = draw.textbbox((0, 0), str(num), font=fnt3)
            w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
    ...
    if args.mode == 'pe':
        ...
        for e in insertions:
            ...
            while x_p in range(120, 900):
                ...
                bbox = draw.textbbox((0, 0), str(ruler), font=fnt3)
                w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
...
def gene_plot(): 
    ...
    for p in intermediate_list:
        ...
        if args.my_mut == 'snp' or args.my_mut == 'dens' or args.my_mut == 'vars' or args.my_mut == 'qtl':
            bbox = draw.textbbox((0, 0), str(scale_tag), font=fnt2)
            w, h = bbox[2] - bbox[0], bbox[3] - bbox[1]
        ...
        if args.my_mut == 'snp' or args.my_mut == 'dens' or args.my_mut == 'vars' or args.my_mut == 'qtl':
            ...
            msg_1 = str(p[4][3])
            msg_2 = str(p[4][4])
            bbox1 = draw.textbbox((0, 0), msg_1, font=fnt4)
            bbox2 = draw.textbbox((0, 0), msg_2, font=fnt4)
            ...
            #Arrows
            #Base
            image_file = BytesIO(open("./fonts/arrowright.png",'rb').read())
            arrow = Image.open(image_file)
            arrow = arrow.resize((47, 47), Image.Resampling.LANCZOS)
