...
#python3 ./report.py -log ./in/log.log -output_html ./out/report.html -project user_projects/project  -mut_type dens -files_dir ./in/
import argparse
...
if mut_type == 'dens' or mut_type == "vars" or mut_type == "qtl": 
	if reads_type == 'pe':
		output.write(
'		<tr>' + '\n'
'			<td> <b>Input test files:</b></td>' + '\n'
...
#__________________________________QTL cartographic report________________________________________________________________
if mut_type == 'qtl':
	#Chromosomes FA vs POS
	#Mapping
	output.write(
		...
		with open(input_var) as candidates:
			...
			for line in candidates:
				if not line.startswith('@'):
					...
					contig = str(sp[1]).strip()
					position = str(sp[2]).strip()
					AF_test = str(sp[8]).strip()
					AF_control = str(sp[9]).strip()
					dAF = str(sp[10]).strip()
					DTP = str(sp[11]).strip()
					nucleotide = str(sp[3]).strip() + ' &rarr; ' + str(sp[4]).strip()
					alt_nt = str(sp[4]).strip()
					...
					downstream = str(sp[27]).strip()
					if str(sp[16]).strip() != '-':
						gene = str(sp[16]).strip() + ' (' + str(sp[17]).strip() + ')'
					else:
						gene = '-'
					if str(sp[19]).strip() != '-':
						annotation = str(sp[21]).strip() 
					else:
						annotation = ' Functional annotation not available'
...
# is used to feed view-report.php in the web-interface with the correct path to resources (images, files, links) 
										...
										#	< IN-PROGRAM
#htmlFile2 = open('3_workflow_output/rfeed.html', 'w')
														# < TESTING	
try: output.close()
except: pass
with open(output_html, 'r') as htmlFile1, open('user_projects/' + project_name + '/3_workflow_output/rfeed.html', 'w') as htmlFile2:
