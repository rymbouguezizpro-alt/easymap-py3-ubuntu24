        ...
        for test_Gene_names in locus_list:
            ...
            def Gene_count_cmp(a, b):
                ...
                if a[0] < b[0]:
                    ...
                    return 1
            Gene_counts = sorted(Gene_counts, key=cmp_to_key(Gene_count_cmp))
            ...
            if base_fname == "hla":
                ...
                if perform_typing_primary_exon:
                    ...
                    if len(primary_exon_alleles) > 0:
                        ...
                        Gene_prob = exon_prob = sorted(exon_prob, key=cmp_to_key(typing_common.Gene_prob_cmp))
                else:
                    # Incorporate representative alleles for exons
                ...
                if len(exon_alleles) > 0:
                    ...
                    Gene_prob = sorted(Gene_prob, key=cmp_to_key(typing_common.Gene_prob_cmp))
            else:
            ...
            for node_name, node in list(asm_graph.nodes.items()):
                ...
                for f_ in [sys.stderr, report_file]:
                    ...
                    for max_allele_name in max_allele_names:
                        ...
                        cmp_vars = sorted(cmp_vars, key=lambda x: int(x[2:]))
                        print_output = False
            ...
            for node_name, node in asm_graph.nodes.items():
                ...
                for f_ in [sys.stderr, report_file]:
                    print("Exonic:", node_name, file=f_)
                    for max_allele_name in max_allele_names:
                        print("\t\t%s:" % max_allele_name, max_common, file=f_)
            """
