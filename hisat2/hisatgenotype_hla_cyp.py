...
def align_reads(ex_path,
    ...
    sambam_proc = subprocess.Popen(sambam_cmd,
                                   ...
                                   stdout=open("hla_input_unsorted.bam", 'wb'),
                                   stderr=open("/dev/null", 'w'))
...
def single_abundance(HLA_cmpt,
    ...
    HLA_prob = sorted(HLA_prob, key=cmp_to_key(HLA_prob_cmp))
    return HLA_prob
...
def joint_abundance(HLA_cmpt,
    ...
    def choose_top_alleles(HLA_prob):
        ...
        HLA_prob_list = sorted(HLA_prob_list, key=cmp_to_key(HLA_prob_cmp))
        HLA_prob = {}
        ...
        for test_HLA_names in hla_list:
            ...
            def HLA_count_cmp(a, b):
                ...
                if a[0] < b[0]:
                    ...
                    return 1
            HLA_counts = sorted(HLA_counts, key=cmp_to_key(HLA_count_cmp))
                    ...
                    for test_HLA_name in test_HLA_names:
                        if count[0] == test_HLA_name:
                            ...
                            if count_i > 0 and HLA_counts[0][1] > count[1]:
                                print("Warning: %s ranked first (count: %d)" % (HLA_counts[0][0], HLA_counts[0][1]), file=sys.stderr)
                                assert False
                            else:
                                test_passed += 1
                            """
            ...
            if len(test_HLA_names) == 2 or not simulation:
                ...
                if os.path.exists(li_hla):
                    ...
                    for line in li_hla_proc.stdout:
                        ...
                        if simulation:
                            if allele1 in test_HLA_names and allele2 in test_HLA_names:
                                print("\t\t\t*** 1 ranked %s-%s (score: %.2f)" % (allele1, allele2, score), file=sys.stderr)
                                success[0] = True
                            else:
                                print("\t\t\tLiModel fails", file=sys.stderr)
                        if best_alleles:
                            print("LiModel %s-%s (score: %.2f)" % (allele1, allele2, score), file=sys.stdout)
