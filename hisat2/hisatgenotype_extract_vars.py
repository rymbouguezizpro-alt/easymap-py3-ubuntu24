...
def extract_vars(base_fname,
    ...
    for gene, ref_gene in list(genes.items()):
        ...
        for name, vars in list(Vars_.items()):
            Vars_[name] = sorted(vars, key=cmp_to_key(cmp_varKey))
        ...
        keys = sorted(list(Vars.keys()), key=cmp_to_key(cmp_varKey))
        var2ID = {}
        ...
        while i < len(keys):
            ...
            for allele in alleles:
                ...
                allele_cur_vars = '#'.join(sorted(list(cur_vars & allele_vars), key=cmp_to_key(cmp_varKey)))
            ...
            haplotypes = sorted(list(haplotypes), key=cmp_to_key(cmp_haplotype))
            ...
            if len(cur_vars) in range(0, 1000):
                # print "vars:", sorted(list(cur_vars), key=cmp_to_key(cmp_varKey))
