...
if mode == 'se':
	...
	for genome_index, template_genome in enumerate(template_genomes):
		...
		while read_count <= number_of_reads_per_genome:
			...
			read_seq = seq_template[read_start-1:read_end-1]                               #My values are 1-based, but python3 is 0-based. I compensate subtracting one position.
...
if mode == 'pe':
	...
	for genome_index, template_genome in enumerate(template_genomes):
		...
		while fragment_count <= number_of_reads_per_genome / 2:
			...
			read_for_seq = seq_template[fragment_start-1 : fragment_start+read_length_for-1]    #My values are 1-based, but python3 is 0-based. I compensate subtracting one position.
