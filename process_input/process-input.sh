...
if [ $data_source == 'exp' ]; then
	if [ $lib_type_sample == 'se' ]; then
		...
		if [[ "$read_s" == *".fq" ]] || [[ "$read_s" == *".fastq" ]] ; then			
			...
			match_vcf=`python3 process_input/verify-input.py -vcf_match $read_s -gff_match $gff_file 2>> $my_log_file` 
	...
	if [ $analysis_type == 'snp' ] || [ $analysis_type == 'dens' ] || [ $analysis_type == 'vars' ] || [ $analysis_type == 'qtl' ] ; then
		if [ $lib_type_ctrl == 'se' ]; then
			if [[ "$read_s_ctrl" == *".fq" ]] || [[ "$read_s_ctrl" == *".fastq" ]] ; then			
				...
				# Check contigs match between VCF and gff3 files
				match_vcf=`python3 process_input/verify-input.py -vcf_match $read_s_ctrl -gff_match $gff_file 2>> $my_log_file` 
